...
mtServiceRoomDeskThree::mtServiceRoomDeskThree()
:m_pkQueueUser(NULL),
...
int mtServiceRoomDeskThree::init( void* pData )
{
...
int mtServiceRoomDeskThree::run( DataRun* pkDataRun )
{
	...
	MT_DEBUG_PTCL("\n组成三人桌子！[U1:%x,%d] [U2:%x %d] [U3:%x,%d]",pkQueueDeskDataNode->pkQueueUserDataNode[0],
															  ...
															  pkQueueDeskDataNode->pkQueueUserDataNode[2]->lUserId);
	...
	kDataWrite.lServiceType      = mtServiceMgr::E_SERVICE_ROOM_DESK_THREE;
	...
	{
		playThreeDeskAi();
		if(pkQueueDeskDataNode->plIsWaitStart[iPlayerIdx] != 1) 
		{
			runThreeDeskWaitStartRecvAi(pkQueueDeskDataNode->pkQueueUserDataNode[iPlayerIdx]);
		}
	}

	else /// 真实用户
	{
		pkQueuePacketDataNode = (mtQueuePacket::DataNode*)m_pkQueuePacket->popInitDataNode("deskThree62");
		setWriteData(pkQueueDeskDataNode->pkQueueUserDataNode[iPlayerIdx + 1], pkQueueDeskDataNode->pkQueueUserDataNode[iPlayerIdx + 2], &kDataWrite);
	...
	{
		pkQueuePacketDataNode = (mtQueuePacket::DataNode*)m_pkQueuePacket->popInitDataNode("deskThree79");
		setWriteData(pkQueueDeskDataNode->pkQueueUserDataNode[iPlayerIdx + 1], pkQueueDeskDataNode->pkQueueUserDataNode[iPlayerIdx - 1], &kDataWrite);
		memcpy(pkQueuePacketDataNode->pcData, &kDataWrite, kDataWrite.lStructBytes);
		pkQueuePacketDataNode->kWSABuf.len  = kDataWrite.lStructBytes;
	...
	{
		pkQueuePacketDataNode = (mtQueuePacket::DataNode*)m_pkQueuePacket->popInitDataNode("deskThree98");
		setWriteData(pkQueueDeskDataNode->pkQueueUserDataNode[iPlayerIdx - 2], pkQueueDeskDataNode->pkQueueUserDataNode[iPlayerIdx - 1], &kDataWrite);
		memcpy(pkQueuePacketDataNode->pcData, &kDataWrite, kDataWrite.lStructBytes);
		pkQueuePacketDataNode->kWSABuf.len  = kDataWrite.lStructBytes;
...
int mtServiceRoomDeskThree::exit()
{

	return	0;
}

int mtServiceRoomDeskThree::setWriteData(mtQueueUser::DataNode* pkDataNode1, mtQueueUser::DataNode* pkDataNode2, DataWrite* pkDataWrite)
{
...
int mtServiceRoomDeskThree::runThreeDeskWaitStartRecvAi(mtQueueUser::DataNode* pkQueueUserDataNode)
{
	/// 响应建立三人桌子
	mtQueuePacket::DataNode* pkQueuePacketDataNode = (mtQueuePacket::DataNode*)m_pkQueuePacket->popInitDataNode("deskThree164");
